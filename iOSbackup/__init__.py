...
class iOSbackup(object):
    ...
    def __repr__(self):
        ...
        return template.format(
            ...
            ManifestKey="Not applicable"
            if iOSbackup.isOlderThaniOS10dot2(
                self.manifest["Lockdown"]["ProductVersion"]
            )
            else self.manifest["ManifestKey"].hex(),
        ...
        for plat in iOSbackup.platformFoldersHint.keys():
            if sys.platform.startswith(plat):
                return os.path.expanduser(
                    os.path.expandvars(iOSbackup.platformFoldersHint[plat])
                )
        return None
    ...
    def getDeviceBasicInfo(udid=None, backuproot=None):
        ...
        if udid and root:
            ...
            try:
                ...
                logging.warning(
                    f"{udid} under {root} doesn't seem to have a manifest file."
                )
                return None
            info = {
                "udid": udid,
                ...
                "date": iOSbackup.convertTime(
                    os.path.getmtime(manifestFile), since2001=False
                ),
            }
        else:
    ...
    def getDeviceList(self, backuproot=None):
        ...
        if self.backupRoot:
            ...
            for i in dirnames:
                list.append(
                    iOSbackup.getDeviceBasicInfo(udid=i, backuproot=self.backupRoot)
                )

            return list
        else:
            raise Exception(
                "Need valid backup root folder path passed through `backuproot`."
            )
    ...
    def getBackupFilesList(self):
        ...
        if not self.manifestDB:
            raise Exception(
                "Object not yet innitialized or can't find decrypted files catalog ({})".format(
                    iOSbackup.catalog["manifestDB"]
                )
            )
        ...
        backupFiles = (
            catalog.cursor()
            .execute(f"SELECT * FROM Files ORDER BY domain,relativePath")
            .fetchall()
        )
    ...
    def getFolderDecryptedCopy_OldInneficientDeprecated(
        self,
        relativePath=None,
        targetFolder=None,
        temporary=False,
        ...
        if includeDomains:
            if type(includeDomains) == list:
                additionalFilters.append(
                    "domain IN ({})".format(
                        ",".join("'" + item + "'" for item in includeDomains)
                    )
                )
            else:
        ...
        if excludeDomains:
            if type(excludeDomains) == list:
                additionalFilters.append(
                    "domain NOT IN ({})".format(
                        ",".join("'" + item + "'" for item in excludeDomains)
                    )
                )
            else:
        ...
        query = "SELECT * FROM Files WHERE relativePath LIKE '{relativePath}%' {additionalFilters} ORDER BY domain, relativePath".format(
            relativePath=relativePath, additionalFilters=" AND ".join(additionalFilters)
        )
        ...
        for f in backupFiles:
            info = {}
            (info, decrypted) = self.getFileDecryptedData(
                fileNameHash=f["fileID"], manifestData=f["file"]
            )

            physicalTarget = os.path.join(
                targetRootFolder, f["domain"], f["relativePath"]
            )
    ...
    def getFolderDecryptedCopy(
        ...
        for payload in backupFiles:
            ...
            physicalTarget = os.path.join(
                targetRootFolder, payload["domain"], payload["relativePath"]
            )
    ...
    def getDomains(self):
        ...
        domains = (
            catalog.cursor().execute(f"SELECT DISTINCT domain FROM Files").fetchall()
        )
    ...
    def getFileManifestDBEntry(self, fileNameHash=None, relativePath=None):
        ...
        if relativePath:
            backupFile = (
                catalog.cursor()
                .execute(
                    f"SELECT * FROM Files WHERE relativePath='{relativePath}' ORDER BY domain LIMIT 1"
                )
                .fetchone()
            )
        else:
            backupFile = (
                catalog.cursor()
                .execute(
                    f"SELECT * FROM Files WHERE fileID='{fileNameHash}' ORDER BY domain LIMIT 1"
                )
                .fetchone()
            )
        ...
        if backupFile:
            ...
            #             payload['manifest']=biplist.readPlistFromString(payload['file'])
            #             payload['manifest']=plistlib.loads(payload['file'])
            payload["manifest"] = NSKeyedUnArchiver.unserializeNSKeyedArchiver(
                payload["file"]
            )
            del payload["file"]
        else:
            if relativePath:
                raise (
                    FileNotFoundError(
                        f"Can't find backup entry for relative path «{relativePath}» on catalog"
                    )
                )
            else:
                raise (
                    FileNotFoundError(
                        f"Can't find backup entry for «{fileNameHash}» on catalog"
                    )
                )
    ...
    def getFileInfo(manifestData):
        ...
        return {
            ...
            "lastModified": iOSbackup.convertTime(
                fileData["LastModified"], since2001=False
            ),
            "lastStatusChange": iOSbackup.convertTime(
                fileData["LastStatusChange"], since2001=False
            ),
            "mode": fileData["Mode"],
            "isFolder": folder,
    ...
    def getFileDecryptedData(self, fileNameHash, manifestData):
        ...
        if "EncryptionKey" in fileData:
            ...
            with open(
                os.path.join(
                    self.backupRoot, self.udid, fileNameHash[:2], fileNameHash
                ),
                "rb",
            ) as infile:
    ...
    def getRelativePathDecryptedData(self, relativePath):
        ...
        (info, dataDecrypted) = self.getFileDecryptedData(
            fileNameHash=backupFile["fileID"], manifestData=backupFile["manifest"]
        )
    ...
    def getFileDecryptedCopy_OldInneficientDeprecated(
        self, relativePath, targetName=None, targetFolder=None, temporary=False
    ):
        """Returns a dict with filename of a decrypted copy of certain file along with some file information
        ...
        (info, dataDecrypted) = self.getRelativePathDecryptedData(
        ...
        if targetName:
            ...
            fileName = "{domain}~{modifiedPath}".format(
                domain=info["domain"], modifiedPath=relativePath.replace("/", "--")
            )

        if temporary:
            targetFileName = tempfile.NamedTemporaryFile(
                suffix=f"---{fileName}", dir=targetFolder, delete=True
            )
    ...
    def getFileDecryptedCopy(
        ...
        if targetName:
            ...
            fileName = "{domain}~{modifiedPath}".format(
                domain=domain, modifiedPath=relativePath.replace("/", "--")
            )
        ...
        if "EncryptionKey" in fileData:
            ...
            with open(
                ...
                if os.name == "nt":
                    mappedInFile = mmap.mmap(
                        inFile.fileno(), length=0, access=mmap.ACCESS_READ
                    )
                else:
                    mappedInFile = mmap.mmap(
                        inFile.fileno(), length=0, prot=mmap.PROT_READ
                    )
                ...
                with open(targetFileName, "wb") as outFile:
                    ...
                    while True:
                        chunk = mappedInFile[
                            chunkIndex * chunkSize : (chunkIndex + 1) * chunkSize
                        ]
        ...
        if type(timeToConvert) == int or type(timeToConvert) == float:
            ...
            if since2001:
                return datetime.fromtimestamp(
                    timeToConvert + apple2001reference.timestamp(), timezone.utc
                )
            else:
    ...
    def getManifestDB(self):
        ...
        with open(
            os.path.join(self.backupRoot, self.udid, iOSbackup.catalog["manifestDB"]),
            "rb",
        ) as db:
        ...
        file = tempfile.NamedTemporaryFile(
            suffix="--" + iOSbackup.catalog["manifestDB"], delete=False
        )
    ...
    def loadManifest(self):
        ...
        manifestFile = os.path.join(
            self.backupRoot, self.udid, iOSbackup.catalog["manifest"]
        )

        self.date = iOSbackup.convertTime(
            os.path.getmtime(manifestFile), since2001=False
        )
        ...
        statusFile = os.path.join(
            self.backupRoot, self.udid, iOSbackup.catalog["status"]
        )
    ...
    def deriveKeyFromPassword(self, cleanpassword=None):
        ...
        if iOSbackup.isOlderThaniOS10dot2(self.manifest["Lockdown"]["ProductVersion"]):
            ...
            temp = hlib.pbkdf2_hmac(
                "sha256", cleanpassword, self.attrs[b"DPSL"], self.attrs[b"DPIC"], 32
            )

        self.decryptionKey = hlib.pbkdf2_hmac(
            "sha1", temp, self.attrs[b"SALT"], self.attrs[b"ITER"], 32
        )
